...
// Track per-message metadata from planner responses
...
export default function ChatPanel() {
  const { currentProject, messages, setMessages, addMessage, isChatLoading, setIsChatLoading, setIsReadyToBuild, showToast } =
    useAppStore()
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setIsReadyToBuild(response.is_ready_to_build)
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        ...
        {messages.map((msg) => (
          <MessageBubble
            key={msg.id}
            message={msg}
            questions={messageMeta[msg.id]?.questions ?? []}
            suggestions={messageMeta[msg.id]?.suggestions ?? []}
            conflicts={messageMeta[msg.id]?.conflicts ?? []}
